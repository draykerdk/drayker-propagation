...
| `forum.drayker.org` | **Forum** — site próprio (`general-forum`), fora deste componente | — |
...
## 3.3 Forum — site próprio (2026-08-11)

O fórum **não é uma página deste componente**. É um site completo, `Drayker Forum.dc.html`,
publicado pelo repositório `draykerdk/general-forum` em **forum.drayker.org** — cabeçalho,
navegação, rotas e rodapé próprios, mesmo sistema visual.

O que este componente tem a respeito: um item `Forum` na navegação dos dois sites, que
atravessa para `FORUM_SITE`. Nenhum conteúdo de conversa é duplicado aqui. A página de fórum
que chegou a existir dentro da v3 foi removida por inteiro (template, lógica e modelo de
dados) e a chave de cache do GitHub voltou para `drayker-gh-v1`.

Entrega e publicação do site: `handoff/forum-site/PUBLICAR.md`.
