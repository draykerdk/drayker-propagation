(function (root) {
  ...
  var Drayker = root.Drayker;
  if (!Drayker || Drayker.bodies.signalMesh) return;
  ...
  Drayker.bodies.signalMesh = {
    ...
    build: function (ctx) {
      ...
      var glow = layer.appendChild(ctx.mk('circle', { r: 47, fill: BLUE, 'fill-opacity': .13, filter: ctx.blurSoft }));
      var back = layer.appendChild(ctx.mk('path', { fill: 'none', stroke: BLUE, 'stroke-width': .55, 'stroke-opacity': .14, 'stroke-linecap': 'round' }));
      var front = layer.appendChild(ctx.mk('path', { fill: 'none', stroke: BLUE, 'stroke-width': .72, 'stroke-opacity': .5, 'stroke-linecap': 'round' }));
      var nodeLayer = layer.appendChild(ctx.mk('g', { fill: ICE }));
      var pulseLayer = layer.appendChild(ctx.mk('g', { fill: '#FFFFFF' }));
      var nodes = mesh.verts.map(function () { return nodeLayer.appendChild(ctx.mk('circle', { r: 1.45 })); });
      var pulses = [];
      for (var i = 0; i < 14; i++) {
        pulses.push({
          phase: i / 14,
